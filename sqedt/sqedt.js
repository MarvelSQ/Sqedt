...
SQEdt.prototype = {
    ...
    addTag: function (node, nodename) {
        return nodename === 'b' ? this.addBoldTag(node) : nodename === 'i' ? this.addItalyTag(node) : nodename == 's' ? this.addSlashTag(node) : ''
    },
    addBoldTag: function (node) {//node must be span
        node.innerHTML = '<b>' + node.innerHTML.replace('<b>', '').replace('</b>', '') + '</b>'
    },
    addItalyTag: function (node) {//node must be span
    ...
    addSlashTag: function (node) {//node must be span
        for (let i = 0; i < node.childNodes.length; i++) {
            let tmp = node.childNodes[i]
            if (tmp.innerText === '') {
                node.replaceChild(document.createTextNode(''), tmp)
