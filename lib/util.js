...
var isAllElements = function isAllElements(type, el) {
  if (el.innerText !== '') {
    ...
    [].concat(_toConsumableArray(elc.querySelectorAll(type))).forEach(function (e) {
      return e.remove();
    });
...
var removeEmpty = function removeEmpty(collection) {
  ...
  try {
    for (var _iterator = removeList.entries()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function replaceWidthChild(element) {
  if (element.parentNode && element.childNodes) {
    var newFrag = document.createDocumentFragment();
    [].concat(_toConsumableArray(element.childNodes)).forEach(function (e) {
...
function camlToHypen(str) {
  return str.replace(/([A-Z])/g, function (word, i) {
    return '-' + word.toLowerCase();
  });
}

exports.default = {
  replaceElementIn: replaceElementIn,
  replaceWidthChild: replaceWidthChild,
  wrapParent: wrapParent,
  getParent: getParent,
  camlToHypen: camlToHypen
};